...
// Funções específicas das novas seções
...
function addRecipe() {
    ...
    const newRecipe = template.cloneNode(true);

    // Atualizar título
    newRecipe.querySelector('h3').textContent = `Receita #${recipeCount}`;

    // Limpar valores
    newRecipe.querySelectorAll('input, select, textarea').forEach(field => {
...
function removeRecipe(button) {
    const recipe = button.closest('.recipe-card');
    const container = recipe.parentElement;

    if (container.children.length > 1) {
        recipe.remove();
        // Renumerar receitas
        container.querySelectorAll('.recipe-card').forEach((card, index) => {
            card.querySelector('h3').textContent = `Receita #${index + 1}`;
        });
    } else {
        formularioVegetal?.showNotification('Deve manter pelo menos uma receita', 'warning');
    }
}

function toggleFertilizer(checkbox) {
    const details = checkbox.parentElement.nextElementSibling;
    if (details && details.classList.contains('fertilizer-details')) {
        if (checkbox.checked) {
            details.style.display = 'block';
        } else {
            details.style.display = 'none';
            formularioVegetal?.clearFields(details);
        }
    }
}

function addCompostIngredient() {
    const tbody = document.querySelector('#compost-table tbody');
    if (!tbody) return;
...
function removeRow(button) {
    ...
    if (tbody.children.length > 1) {
        ...
        formularioVegetal?.showNotification('Deve manter pelo menos um item', 'warning');
    }
}
...
function showProductTab(tabName) {
    ...
    // Aqui você pode filtrar a tabela por tipo de produto
    console.log('Mostrar produtos:', tabName);
}
...
function removeProductRow(button) {
    ...
    if (tbody.children.length > 1) {
        ...
        formularioVegetal?.showNotification('Deve manter pelo menos um produto', 'warning');
    }
}

function showThirdPartyDetails(show) {
    const details = document.getElementById('third-party-details');
    if (details) {
...
function showLendingDetails(show) {
    const details = document.getElementById('lending-details');
    if (details) {
        details.style.display = show ? 'block' : 'none';
        if (!show) {
            formularioVegetal?.clearFields(details);
...
function toggleOtherCleaning(checkbox) {
    const otherInput = document.querySelector('[name="limp_outro_qual"]');
    if (otherInput) {
        if (checkbox.checked) {
...
function updateStorageRecommendations(select) {
    const recommendations = document.getElementById('storage-recommendations');
    ...
    switch (select.value) {
        case 'mediana':
            content = `
                <div class="recommendations-content">
                    <h4>📋 Recomendações para Melhoria:</h4>
                    <ul>
                        <li>Organizar produtos por categoria</li>
                        <li>Etiquetar todos os recipientes</li>
                        <li>Melhorar ventilação do espaço</li>
                        <li>Criar separação física entre tipos de produtos</li>
                    </ul>
                </div>
            `;
        ...
        case 'ruim':
            content = `
                <div class="recommendations-content urgent">
                    <h4>🚨 Ações Urgentes Necessárias:</h4>
                    <ul>
                        <li><strong>Separar imediatamente</strong> produtos químicos de insumos orgânicos</li>
                        <li><strong>Etiquetar todos</strong> os produtos claramente</li>
                        <li><strong>Limpar e organizar</strong> todo o espaço</li>
                        <li><strong>Instalar ventilação</strong> adequada</li>
                        <li><strong>Criar sistema de controle</strong> de pragas</li>
                        <li><strong>Documentar</strong> todas as melhorias realizadas</li>
                    </ul>
                    <p class="alert">⚠️ Organização inadequada pode resultar em não-conformidade!</p>
                </div>
            `;
            recommendations.style.display = 'block';
            break;
...
function previewStoragePhotos(input) {
    const preview = document.getElementById('storage-photos-preview');
    if (!preview) return;
...
function addNonCertifiedRow() {
    const tbody = document.getElementById('nao-certificados-body');
    if (!tbody) return;

    const template = tbody.querySelector('.non-certified-row');
    const newRow = template.cloneNode(true);
    const rowNum = tbody.children.length + 1;

    // Atualizar número da linha
    newRow.querySelector('.row-num').textContent = rowNum;
    ...
    // Esconder campo "outro motivo"
    const otherField = newRow.querySelector('[name="nc_motivo_outro[]"]');
    if (otherField) {
        otherField.style.display = 'none';
    }
...
function removeNonCertified(button) {
    const row = button.closest('tr');
    const tbody = row.parentElement;
    ...
    if (tbody.children.length > 1) {
        row.remove();
        // Renumerar linhas
        tbody.querySelectorAll('.row-num').forEach((cell, index) => {
...
function checkOtherReason(select) {
    const otherInput = select.parentElement.querySelector('[name="nc_motivo_outro[]"]');
    if (otherInput) {
...
function updateCompostDetails(select) {
    // Função para mostrar campos específicos baseado no método de compostagem
    console.log('Método de compostagem selecionado:', select.value);
}
